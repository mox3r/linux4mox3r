...
echo ":: Generate new cached wallpaper blur-$blur-default with blur $blur"
magick $wallpaper -resize 75% $blurred_wallpaper
echo ":: Resized to 75%"
if [ ! "$blur" == "0x0" ] ;then
    magick $blurred_wallpaper -blur $blur $blurred_wallpaper
    cp $blurred_wallpaper $generated_versions/blur-$blur-default.png
    echo ":: Blurred"
fi
cp $generated_versions/blur-$blur-default.png $blurred_wallpaper
...
echo ":: Generate new cached wallpaper square-default"
magick $wallpaper -gravity Center -extent 1:1 $square_wallpaper
cp $square_wallpaper $generated_versions/square-default.png
